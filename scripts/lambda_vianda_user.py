...
def lambda_handler(event, context):
    try:
        print("Lambda USER VIANDAS iniciada")
        ...
        # Obtener las viandas del usuario (tanto las que creó como las que compró)
        query = """
            SELECT 
                v.id, 
                v.titulo, 
                ...
                CASE 
                    WHEN v.fk_dueno = %s THEN 'creada'
                    WHEN cv.id IS NOT NULL THEN 'comprada'
                    ELSE 'otra'
                END as tipo
            FROM vianda v
            JOIN persona p ON v.fk_dueno = p.id
            LEFT JOIN compra_vianda cv ON v.id = cv.fk_vianda AND cv.fk_comprador = %s
            WHERE v.fk_dueno = %s OR cv.fk_comprador = %s
            ORDER BY v.id DESC
        """
        
        print("Ejecutando query de viandas del usuario")
        ...
        for vianda in viandas:
            viandas_list.append({
                ...
                'tipo': vianda[8]  # 'creada' o 'comprada'
            })
        
        cur.close()
        conn.close()
        
        print(f"Se encontraron {len(viandas_list)} viandas para el usuario")
        ...
        return {
            ...
            'body': json.dumps({
                'error': 'Error al obtener las viandas del usuario',
                'detalles': str(e)
            })
        } 